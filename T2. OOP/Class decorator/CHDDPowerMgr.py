...
# Current system requirements changed.
# gem_utils.py --mode=cli --clicommand="command" is a multyfunctional tool for all system
# controller_type, serial setup not used any more and need to be deleted safely
# Clean up code, check return code logic should be changed, reset_drive is off/on state.
# wait for power cmd return code and check it.
"""
#  CHDDPowerMgr.py
#  Laguna Seca & SATI-TL controllers power off/on drive support
#  Values names used across multiple scripts
"""
import time
...
class CHDDPowerMgr(object):
    ...
    LS_RESET_COMMAND = r'/avid/tools/bin/gem_utils.py --mode=cli --clicommand="resetdrive {}"'  # format with slot number/for both controllers type 
